...
#-------------------------------------------------
    ...
    #-------------------------------------------------------
    # Test 1: Positive < Positive (5 < 10 → branch taken)
    #-------------------------------------------------------
    li   x2, 5
    li   x3, 10
    blt  x2, x3, L1
...
L1: addi x11, x11, 1     # executed → 1
    ...
    #-------------------------------------------------------
    # Test 2: Positive < Positive (10 < 5 → not taken)
    #-------------------------------------------------------
    li   x2, 10
    li   x3, 5
    blt  x2, x3, L2
...
L2: addi x13, x13, 1     # executed → 1
    ...
    #-------------------------------------------------------
    # Test 3: Negative < Positive (-1 < 5 → branch taken)
    #-------------------------------------------------------
    li   x2, -1
    li   x3, 5
    blt  x2, x3, L3
...
L3: addi x15, x15, 1     # executed → 1
    ...
    #-------------------------------------------------------
    # Test 4: Positive < Negative (5 < -1 → not taken)
    #-------------------------------------------------------
    li   x2, 5
    li   x3, -1
    blt  x2, x3, L4
    addi x16, x16, 1     # executed → 1
...
L5: addi x19, x19, 1     # executed → 1
    ...
    # x4=20+5=25, x5=30 → (25<30 → taken)
    #-------------------------------------------------------
    li   x4, 20
    li   x5, 30
    addi x4, x4, 5       # ALU op writes x4
    blt  x4, x5, L6
...
L6: addi x21, x21, 1     # executed → 1
    ...
    # x6 loaded = 0xFF, x7 = 0x100 → 0xFF<0x100 → taken
    #-------------------------------------------------------
    li   x8,  0x80000000
    li   x7,  0x00000100
    li   x9,  0x000000FF
    sw   x9,  0(x8)
    lw   x6,  0(x8)      # LOAD value into x6
    blt  x6, x7, L7
...
L7: addi x23, x23, 1     # executed → 1
    ...
    # x9 loaded = 0xFF, x7 = 0xFF → equal → not taken
    #-------------------------------------------------------
    li   x7,  0x000000FF
    li   x9,  0
    sw   x7,  0(x8)
...
L8: addi x25, x25, 1     # executed → 1
    ...
    #-------------------------------------------------------
    # Test 9: Backward branch (loop) until counter==5
    #-------------------------------------------------------
    li   x1,  0
    li   x3,  5
Loop:
    addi x1, x1, 1
    blt  x1, x3, Loop    # branch while x1<5
    add  x26, x26, x1    # after loop → 5

    #-------------------------------------------------------
    # Test 10: Chain of branches
    # x7=5, x8=5 → equal → not taken
    # x7=5, x9=10 → 5<10 → taken
    #-------------------------------------------------------
    li   x7, 5
    li   x8, 5
    li   x9, 10
