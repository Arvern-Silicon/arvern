...
L1: addi x11, x11, 1     # executed → 1
    ...
    #-------------------------------------------------------
    # Test 2: Unsigned (5 >= 10 → not taken)
    #-------------------------------------------------------
    li   x2, 5
    li   x3, 10
    bgeu x2, x3, L2
...
L2: addi x13, x13, 1     # executed → 1
    ...
    #-------------------------------------------------------
    # Test 3: Unsigned (0xFFFFFFFF >= 5 → taken)
    #-------------------------------------------------------
    li   x2, -1          # 0xFFFFFFFF
    li   x3, 5
    bgeu x2, x3, L3
...
L3: addi x15, x15, 1     # executed → 1
    ...
    #-------------------------------------------------------
    # Test 4: Unsigned (5 >= 0xFFFFFFFF → not taken)
    #-------------------------------------------------------
    li   x2, 5
    li   x3, -1          # 0xFFFFFFFF
    bgeu x2, x3, L4
...
L6: addi x21, x21, 1     # executed → 1
    ...
    # x6 = 0xFF, x7 = 0x100 (unsigned → not taken)
    #-------------------------------------------------------
    li   x8,  0x80000000
    li   x7,  0x00000100
    li   x9,  0x000000FF
...
L8: addi x25, x25, 1     # executed → 1
    ...
    #-------------------------------------------------------
    # Test 9: Backward branch loop (unsigned count)
    #-------------------------------------------------------
    li   x1, 16
    li   x2, 10
Loop:
    addi x1, x1, -1
    bgeu x1, x2, Loop     # loop until x1 < 1
    add  x26, x26, x1     # after loop → 9

    #-------------------------------------------------------
    # Test 10: Chain of branches
    # x7=5, x8=5 → equal → taken
    # x7=5, x9=10 → 5>=10 (unsigned → not taken)
    #-------------------------------------------------------
    li   x7, 5
    li   x8, 5
    li   x9, 10
