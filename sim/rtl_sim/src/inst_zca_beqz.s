...
main:
    ...
    # Encoding: 110_offset[8|4:3]_rs1'[2:0]_offset[7:6|2:1|5]_01
    #
    # Key behavior:
...
.option rvc          # enable compressed instructions
    ...
    #-------------------------------------------------
    # Test Set 1: Branch taken (rs1' = 0)
    #-------------------------------------------------
test_1:
    addi x18, x18, 1     # Test counter = 1
    li   x8, 0           # Set x8 = 0
    c.beqz x8, test_1_taken
...
test_1_taken:
    ...
    #-------------------------------------------------
    # Test Set 2: Branch not taken (rs1' = 1)
    #-------------------------------------------------
test_2:
    addi x18, x18, 1     # Test counter = 2
    li   x8, 1           # Set x8 = 1 (non-zero)
    c.beqz x8, test_2_taken
...
test_2_end:
    ...
    #-------------------------------------------------
    # Test Set 3: Branch not taken (rs1' = positive)
    #-------------------------------------------------
test_3:
    addi x18, x18, 1     # Test counter = 3
    li   x9, 0x12345678  # Set x9 = positive value
    c.beqz x9, test_3_taken
    addi x17, x17, 1     # Not-taken counter = 2
    j    test_3_end
...
test_3_end:
    ...
    #-------------------------------------------------
    # Test Set 4: Branch not taken (rs1' = negative)
    #-------------------------------------------------
test_4:
    addi x18, x18, 1     # Test counter = 4
    li   x10, 0xFFFFFFFF # Set x10 = -1 (negative)
    c.beqz x10, test_4_taken
...
test_4_end:
    ...
    #-------------------------------------------------
    # Test Set 5: Multiple branches with zero
    #-------------------------------------------------
test_5:
...
test_5_b:
    ...
    #-------------------------------------------------
    # Test Set 6: Backward branch (simple loop)
    #-------------------------------------------------
test_6:
...
test_6_loop:
    ...
    bnez x12, test_6_loop # Loop while not zero (standard instruction)
.option rvc
    c.beqz x12, test_6_end # Branch when zero (testing c.beqz)
    addi x17, x17, 1     # Should be skipped
...
test_8_a:
    ...
    li   x8, 1           # Non-zero
    c.beqz x8, test_8_b
    addi x17, x17, 1     # Not-taken counter = 4
    j    test_8_c
...
test_10:
    ...
    c.beqz x10, test_10_end
    addi x17, x17, 1     # Should be skipped

test_10_end:
    addi x16, x16, 1     # Taken counter = 16
    addi x21, x10, 0     # Backup x10 after branch (should still be 0)
...
test_11:
    ...
    li   x8, 0           # Set x8 = 0 (will take branch)
    c.beqz x8, test_11_max_forward  # Branch +254 bytes (maximum positive offset)
    addi x17, x17, 1     # Should be skipped

    # Fill space to create exactly +254 byte offset
    # 254 bytes = 127 half-word slots
    # c.beqz instruction = 2 bytes, so we need 125 more half-words
    .rept 125
    c.nop                # 2-byte NOPs to fill space
    .endr

test_11_max_forward:
    addi x16, x16, 1     # Taken counter = 17 (max positive offset success)
...
test_12_max_backward:
    addi x16, x16, 1     # Taken counter = 18 (max negative offset success)
...
test_12_jump_point:
    li   x9, 0           # Set x9 = 0 (will take branch)
    c.beqz x9, test_12_max_backward  # Branch -256 bytes (maximum negative offset)
    addi x17, x17, 1     # Should be skipped
...
.option norvc
    ...
    addi x22, x16, 0     # Backup taken counter (should be 18)
    addi x23, x17, 0     # Backup not-taken counter (should be 4)
    addi x24, x18, 0     # Backup test counter (should be 12)
    addi x25, x19, 0     # Backup loop body counter (should be 3)
    addi x26, x20, 0     # Backup x10 before branch (should be 0)
    addi x27, x21, 0     # Backup x10 after branch (should be 0)
