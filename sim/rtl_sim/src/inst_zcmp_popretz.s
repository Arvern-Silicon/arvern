...
main:
    ...
    # a0 should be zeroed after cm.popretz
    #-------------------------------------------------
    ...
    li   x1,  0                       # Clear ra
    li   x10, 0xA0A0A0A0              # Set a0 to non-zero (should be zeroed)
    li   x5,  0xCAFECAFE              # Delay-slot canary
    cm.popretz {ra}, 16               # Pop ra, zero a0, and return
...
check_test1:
    ...
    # Check a0 == 0 (must be zeroed by cm.popretz)
    li   x30, 0x00000102
    mv   x11, x10
    li   x12, 0x00000000
    bne  x11, x12, test_fail
    ...
    li   x1,  0                       # Clear ra
    li   x8,  0                       # Clear s0
    li   x10, 0xA0A0A0A0              # Set a0 to non-zero
    li   x5,  0xCAFECAFE              # Delay-slot canary
    cm.popretz {ra, s0}, 32           # Pop ra,s0, zero a0, and return
...
check_test13:
    ...
    #-------------------------------------------------
    # Test 14a: Push-popretz roundtrip
    # SP=0x8000E000
    # Set ra=check_test14a, s0=test_val, s1=test_val
    # cm.push {ra, s0-s1}, -48 -> SP=0x8000DFD0
    # Clear regs, set a0 non-zero
    # cm.popretz {ra, s0-s1}, 48 -> SP=0x8000E000, return to check_test14a
    #-------------------------------------------------
    ...
    li   x10, 0xA0A0A0A0              # Set a0 to non-zero
    li   x5,  0xCAFECAFE              # Delay-slot canary
    cm.popretz {ra, s0-s1}, 48        # Pop, zero a0, and return to check_test14a
...
check_test14a:
    ...
    sw   x4,  28(x3)
    la   x4,  check_test14b           # ra for popretz = check label
    ...
    # Clear ra for popretz
    li   x1,  0

    li   x10, 0xA0A0A0A0              # Set a0 to non-zero
    li   x5,  0xCAFECAFE              # Delay-slot canary
    cm.popretz {ra}, 16               # Pop ra, zero a0, and return, SP: 0x8000E120 -> 0x8000E130
...
check_test14b:
    ...
    #-------------------------------------------------
    # Test 15a: RAW on SP input - li x2, addr; cm.popretz
    # SP=0x8000F000, {ra}, 16
    #-------------------------------------------------
    ...
    li   x10, 0xA0A0A0A0              # Set a0 to non-zero
    li   x5,  0xCAFECAFE              # Delay-slot canary
    li   x2,  0x8000F000              # Set SP immediately before popretz
    cm.popretz {ra}, 16               # Must use SP=0x8000F000
...
check_test15a:
    ...
    #-------------------------------------------------
    # Test 15b: Load-use on SP - lw x2, 0(x3); cm.popretz
    # SP=0x8000F100, {ra}, 16
    #-------------------------------------------------
    li   x3,  0x8000F100
...
check_test15b:
    ...
    #-------------------------------------------------
    # Test 15c: Store then cm.popretz to overlapping address
    # (cm.popretz must read the value just written by sw)
    # SP=0x8000F200
    #-------------------------------------------------
    li   x3,  0x8000F200
    ...
    sw   x4,  12(x2)                  # Store real ra RIGHT BEFORE cm.popretz
    li   x1,  0                       # Clear ra
    li   x10, 0xA0A0A0A0              # Set a0 to non-zero
    li   x5,  0xCAFECAFE              # Delay-slot canary
    cm.popretz {ra}, 16               # Must load the just-stored value
...
check_test15c:
    ...
    # cm.popretz {ra, s0} must NOT clobber s1-s11
    # Must zero a0
    # SP=0x8000F300
    #-------------------------------------------------
...
check_test15d:
    ...
    #-------------------------------------------------
    # Test 15e: Push-popretz roundtrip with full rlist
    # SP=0x8000F400
    # Push {ra, s0-s1}, -48 -> SP=0x8000F3D0
    # Clear regs, set a0 non-zero
    # Popretz {ra, s0-s1}, 48 -> SP=0x8000F400 (restored), return
    #-------------------------------------------------
    ...
    li   x10, 0xA0A0A0A0              # Set a0 to non-zero
    li   x5,  0xCAFECAFE              # Delay-slot canary
    cm.popretz {ra, s0-s1}, 48        # Pop, zero a0, and return to check_test15e
...
check_test15e:
    ...
    # cm.popretz returns to addr where addr%4==2, and the
...
check_test16c:
    ...
    #-------------------------------------------------
    # Test 16d: Push-popretz roundtrip with non-aligned return
    # SP=0x8000F800
    # Set ra to non-aligned check label, push, clear, popretz
    #-------------------------------------------------
    ...
    li   x10, 0xA0A0A0A0              # Set a0 to non-zero
    li   x5,  0xCAFECAFE              # Delay-slot canary
    cm.popretz {ra, s0}, 32           # Pop, zero a0, and return to non-aligned address
...
check_test16d:
    ...
    # li x2, addr; cm.popretz returns to non-aligned addr
    # SP=0x8000F900, {ra}, 16
    #-------------------------------------------------
    ...
    li   x10, 0xA0A0A0A0              # Set a0 to non-zero
    li   x5,  0xCAFECAFE              # Delay-slot canary
    li   x2,  0x8000F900              # Set SP immediately before popretz
    cm.popretz {ra}, 16               # Must use SP=0x8000F900
...
check_test16e:
    ...
    #=========================================================
    # Test 17: CM.POPRETZ-specific a0 zeroing tests
    #=========================================================

    #-------------------------------------------------
    # Test 17a: a0 pre-set to 0xFFFFFFFF
    # Verify a0 is zeroed even from all-ones value
    # SP=0x8000FA00, rlist=4 {ra}, stack_adj=16
    #-------------------------------------------------
    li   x2,  0x8000FA00              # Set SP
    la   x3,  check_test17a
    sw   x3,  12(x2)                  # Store return address at ra position

    li   x1,  0                       # Clear ra
    li   x10, 0xFFFFFFFF              # Set a0 to all-ones
    li   x5,  0xCAFECAFE              # Delay-slot canary
    cm.popretz {ra}, 16               # Pop ra, zero a0, and return
...
check_test17a:
    ...
    # Check ra
    li   x30, 0x00001104
    la   x12, check_test17a
    bne  x1, x12, test_fail


    #-------------------------------------------------
    # Test 17b: a0 pre-set to 0x00000001 (minimal non-zero)
    # Verify a0 is zeroed from smallest positive value
    # SP=0x8000FB00, rlist=5 {ra,s0}, stack_adj=32
    #-------------------------------------------------
    li   x2,  0x8000FB00              # Set SP
    la   x3,  check_test17b
    sw   x3,  24(x2)                  # Store ra at [SP+24]
    li   x3,  0x17B00002              # s0 value
    sw   x3,  28(x2)                  # Store s0 at [SP+28]

    li   x1,  0                       # Clear ra
    li   x8,  0                       # Clear s0
    li   x10, 0x00000001              # Set a0 to minimal non-zero
    li   x5,  0xCAFECAFE              # Delay-slot canary
    cm.popretz {ra, s0}, 32           # Pop ra,s0, zero a0, and return
...
check_test17b:
    ...
    #-------------------------------------------------
    # Test 17c: Push-popretz roundtrip with non-zero a0
    # Verify a0 is zeroed even though it wasn't explicitly pushed
    # SP=0x8000FC00, push {ra, s0-s1}, -48 -> SP=0x8000FBD0
    # a0=0xBEEFBEEF before push, a0=0x12345678 before popretz
    # After popretz: a0 must be 0
    #-------------------------------------------------
    li   x2,  0x8000FC00              # Set SP
    la   x1,  check_test17c           # Return address
    li   x8,  0x17C00002              # s0 test value
    li   x9,  0x17C00003              # s1 test value
    li   x10, 0xBEEFBEEF              # a0 before push (not in rlist)

    cm.push {ra, s0-s1}, -48          # Push: SP -> 0x8000FBD0
    ...
    li   x10, 0x12345678              # a0 still non-zero before popretz
    li   x5,  0xCAFECAFE              # Delay-slot canary
    cm.popretz {ra, s0-s1}, 48        # Pop, zero a0, and return
...
check_test17c:
    ...
    #-------------------------------------------------
    # Test 17d: Verify argument registers a1-a7 untouched
    # cm.popretz must zero a0 but NOT affect a1-a7
    # SP=0x8000FD00, rlist=4 {ra}, stack_adj=16
    #-------------------------------------------------
    li   x2,  0x8000FD00              # Set SP
    la   x3,  check_test17d
    sw   x3,  12(x2)                  # Store return address at ra position

    # Set all argument registers to known non-zero values
    li   x10, 0xDEADBEEF              # a0 (should be zeroed)
    li   x11, 0xB1B1B1B1              # a1 (should be preserved)
    li   x12, 0xB2B2B2B2              # a2 (should be preserved)
    li   x13, 0xB3B3B3B3              # a3 (should be preserved)
    li   x14, 0xB4B4B4B4              # a4 (should be preserved)
    li   x15, 0xB5B5B5B5              # a5 (should be preserved)
    li   x16, 0xB6B6B6B6              # a6 (should be preserved)
    li   x17, 0xB7B7B7B7              # a7 (should be preserved)

    li   x1,  0                       # Clear ra
    li   x5,  0xCAFECAFE              # Delay-slot canary
    cm.popretz {ra}, 16               # Pop ra, zero a0, return
...
check_test17d:
    # Save a1 and a2 (x11, x12) before we use them as comparison temps
    mv   x3, x11                      # Save a1 to x3
    mv   x4, x12                      # Save a2 to x4
    ...
    # Check ra
    li   x30, 0x00001134
    la   x12, check_test17d
    bne  x1, x12, test_fail

    # Check a1 (saved in x3)
    li   x30, 0x00001135
    mv   x11, x3
    li   x12, 0xB1B1B1B1
    bne  x11, x12, test_fail

    # Check a2 (saved in x4)
    li   x30, 0x00001136
    mv   x11, x4
    li   x12, 0xB2B2B2B2
    bne  x11, x12, test_fail
    ...
    #-------------------------------------------------
    # Test 18: IRQ stress test - 20 iterations of
    # cm.popretz {ra, s0-s11}, 64 (13 regs, max kill window)
    # Each iteration: push known values to stack (with
    # ra = check label), set a0 to non-zero, clear regs,
    # popretz them back and verify all 13 registers +
    # a0=0 + return jump.
...
test18_loop:
    li  x30, 0x00120000              # Error code base: Test 18

    # Set all registers to known values based on iteration
    slli x29, x28, 4                  # x29 = iter << 4
    lui  x5, 0x18000                  # x5 = 0x18000000 (base)
    or   x5, x5, x29                 # x5 = 0x18000000 | (iter << 4)

    la   x1,  test18_check            # ra  = return address (check label)
    ...
    # Set a0 to non-zero (popretz must clear it)
    li   x10, 0xA0A0A0A0
    ...
    # Execute longest popretz: 13 registers, maximum IRQ kill window
    # Loads regs from stack, zeroes a0, SP -> 0x8000FE00, jumps to ra
    cm.popretz {ra, s0-s11}, 64

    # Should NOT reach here — popretz jumps to test18_check
    li  x30, 0x00120000
    j   test_fail
...
test18_check:
    ...
    # ---- Verify a0 is zero (the key CM.POPRETZ behavior) ----
    ...
    li   x3,  0x8000FE00             # Expected SP (use x3, not x10 which must stay 0)
    bne  x2,  x3,  test_fail
...
test_fail:
	...
	#   TT = Test number (01-0F, 10-11)
	#   CC = Check number within test (00 = return didn't happen)
	#        01 = delay-slot canary, 02 = a0 zero check
	#        03+ = SP/ra/register checks
