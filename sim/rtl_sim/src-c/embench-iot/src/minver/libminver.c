...
verify_benchmark (int res __attribute ((unused)))
{
  ...
  /* c = a_ref * b: exact integer arithmetic, toolchain-independent */
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++)
      if (fabsf (c[i][j] - c_exp[i][j]) > eps)
	      return 1;

  /* d = inv(a_ref): verify a_ref * d ≈ I instead of hardcoded float values.
     This is toolchain-independent: a_ref has integer entries and the expected
     result (identity matrix) is exactly representable in float. */
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++)
      {
	      float w = 0.0f;
