...
def validate_tools():
    ...
    # Validate C compiler (use first word only — cc may contain flags like
    # "clang --target=riscv32-unknown-elf -fuse-ld=lld")
    cc_exe = gp['cc'].split()[0]
    if not shutil.which(cc_exe):
        log.error('ERROR: Compiler {cc} not found on path: exiting'.format(cc=cc_exe))
        sys.exit(1)

    # Validate linker
    ld_exe = gp['ld'].split()[0]
    if not shutil.which(ld_exe):
        log.error('ERROR: Linker {ld} not found on path: exiting'.format(ld=ld_exe))
        sys.exit(1)
...
def compile_file(f_root, srcdir, bindir, suffix='.c'):
    ...
    # Construct the argument list (split cc in case it contains flags)
    arglist = gp["cc"].split()
